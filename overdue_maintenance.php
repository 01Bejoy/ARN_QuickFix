...
if ($_SERVER["REQUEST_METHOD"] === "POST" && isset($_POST['action_type'])) {
    if ($_POST['action_type'] === 'add_schedule') {
        ...
        $asset_ref_id = (int)($_POST['asset_ref_id'] ?? 0);
        $asset_type = '';
        $asset_id = '';
        ...
        $_SESSION['selected_asset_ref_id'] = $asset_ref_id;
        $_SESSION['form_cache'] = [
        ...
        if (!empty($client_email) && $asset_ref_id > 0 && !empty($next_due) && !empty($maintenance_type)) {
            
            // Asset identity is selected from the canonical asset registry; it is never manually typed.
            $assetVerificationStmt = $conn->prepare("SELECT id, customer_email, customer_id, asset_id, asset_type
                FROM assets
                WHERE id = ?
                  AND LOWER(TRIM(customer_email)) = LOWER(TRIM(?))
                LIMIT 1");
            $assetVerificationStmt->bind_param("is", $asset_ref_id, $client_email);
            $assetVerificationStmt->execute();
            $assetRow = $assetVerificationStmt->get_result()->fetch_assoc();
            $assetVerificationStmt->close();
            $assetCheckResult = ['total' => $assetRow ? 1 : 0];
            if ($assetRow) { $asset_id = $assetRow['asset_id']; $asset_type = $assetRow['asset_type']; }
            ...
            $repairCheckStmt = $conn->prepare("SELECT COUNT(*) AS active_repair FROM service_requests WHERE asset_id = ? AND LOWER(TRIM(status)) = 'processing'");
            $repairCheckStmt->bind_param('s', $asset_id);
            $repairCheckStmt->execute();
            $repairCheckResult = $repairCheckStmt->get_result()->fetch_assoc() ?: ['active_repair' => 0];
            $repairCheckStmt->close();
            ...
            elseif ($status === 'Completed' && $next_due > $todayDateString) {
                ...
                $insertStmt = $conn->prepare("INSERT INTO maintenance_schedules (client_email, client_name, phone, asset_type, asset_id, asset_ref_id, last_service, next_due, maintenance_type, status) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
                $insertStmt->bind_param("sssssissss", $client_email, $client_name, $client_phone, $asset_type, $asset_id, $asset_ref_id, $last_service, $next_due, $maintenance_type, $status);
                
                if ($insertStmt->execute()) {
                    $_SESSION['action_message'] = "Success! Maintenance task logged and published onto client portal lanes.";
                    unset($_SESSION['form_cache'], $_SESSION['selected_asset_ref_id']); // Clear form memory after success
                } else {
                    $_SESSION['action_error'] = "Database Error: Could not save schedule parameters. " . $insertStmt->error;
                }
                $insertStmt->close();
...
// LIVE SYNC: any unfinished schedule whose due date has passed becomes Overdue.
$syncOverdue = $conn->prepare("UPDATE maintenance_schedules
    SET status = 'Overdue'
    WHERE next_due IS NOT NULL
      AND next_due <> '0000-00-00'
      AND next_due < ?
      AND LOWER(TRIM(COALESCE(status,''))) <> 'completed'");
if ($syncOverdue) {
    $syncOverdue->bind_param('s', $currentCalendarDay);
    $syncOverdue->execute();
    $syncOverdue->close();
}

$qOver = $conn->query("SELECT COUNT(*) AS total
    FROM maintenance_schedules
    WHERE LOWER(TRIM(COALESCE(status,''))) = 'overdue'");
if ($qOver) {
    $totalOverdue = (int)($qOver->fetch_assoc()['total'] ?? 0);
}

// Fetch clients to populate select dropdown input field
$clientsList = $conn->query("SELECT id, email, name FROM users WHERE LOWER(TRIM(role)) IN ('client','user','customer') ORDER BY name ASC");
...
$schedulesLedger = $conn->query("SELECT ms.*,
                                         COALESCE(NULLIF(ms.phone,''), (
                                             SELECT sr.phone
                                             FROM service_requests sr
                                             WHERE sr.asset_id = ms.asset_id
                                             ORDER BY sr.id DESC
                                             LIMIT 1
                                         )) AS real_client_phone
...
<head>
  ...
  <script>
    // Client Target Email -> Registered Asset linkage.
    // Email is the authoritative relationship in this project.
    document.addEventListener('DOMContentLoaded', function () {
      const clientSelect = document.querySelector('select[name="client_email"]');
      const assetSelect = document.getElementById('asset_ref_id');
      const assetHelp = document.getElementById('assetHelp');
      if (!clientSelect || !assetSelect) return;

      const allAssets = Array.from(assetSelect.querySelectorAll('option[data-email]')).map(function (o) {
        return {
          value: o.value,
          text: o.textContent.trim(),
          email: (o.getAttribute('data-email') || '').trim().toLowerCase()
        };
      });

      function syncAssets() {
        const email = (clientSelect.value || '').trim().toLowerCase();
        const matches = allAssets.filter(a => a.email === email);
        ...
        placeholder.textContent = matches.length
          ...
          : 'No registered asset found for this client';
        ...
        if (matches.length === 1) {
          assetSelect.value = matches[0].value;
        }

        if (!email) {
          assetHelp.textContent = 'Select a Client Target Email first.';
        } else if (!matches.length) {
          assetHelp.textContent = 'No Registered Asset is linked to this email.';
        } else {
          assetHelp.textContent = matches.length + ' Registered Asset(s) available for this client.';
        }
      }

      if (!clientSelect.value) {
        const requested = Array.from(clientSelect.options).find(
          o => (o.value || '').trim().toLowerCase() === 'brbejoy901@gmail.com'
        );
        if (requested) clientSelect.value = requested.value;
      }

      clientSelect.addEventListener('change', syncAssets);
      syncAssets();
    });
        ...
        <div class="row g-3">
          ...
          <div class="col-md-4">
            ...
            <select name="client_email" class="form-select form-select-custom w-100" required>
              ...
              ?>
                  <option value="<?php echo htmlspecialchars($c['email']); ?>" data-user-id="<?php echo (int)$c['id']; ?>" <?php echo $selected; ?>><?php echo htmlspecialchars($c['name'] . " (" . $c['email'] . ")"); ?></option>
                <?php endwhile; ?>
              <?php endif; ?>
            </select>
          </div>

          <!-- Input 2: Canonical Asset Selector -->
          <div class="col-md-8">
            <label class="form-label form-label-custom">Registered Asset</label>
            <select name="asset_ref_id" id="asset_ref_id" class="form-select form-select-custom w-100" required>
              <option value="" disabled selected hidden>Select registered customer asset</option>
              <?php
              $assetOptions = $conn->query("SELECT a.id, a.asset_id, a.asset_type, a.customer_email, a.customer_id,
                                                   COALESCE(NULLIF(u.name,''), a.customer_email) AS customer_name,
                                                   u.id AS matched_user_id
                                            FROM assets a
                                            LEFT JOIN users u ON u.id = a.customer_id
                                            ORDER BY customer_name ASC, a.asset_id ASC");
              $selectedClientEmail = strtolower(trim($cache['client_email'] ?? ''));
              $selectedAssetId = (int)($_SESSION['selected_asset_ref_id'] ?? 0);
              $assetOptionCount = 0;
              if($assetOptions):
                while($ao=$assetOptions->fetch_assoc()):
              ?>
                <option value="<?php echo (int)$ao['id']; ?>"
                        data-email="<?php echo htmlspecialchars(strtolower(trim($ao['customer_email']))); ?>"
                        data-customer-id="<?php echo (int)($ao['customer_id'] ?? 0); ?>"
                        data-matched-user-id="<?php echo (int)($ao['matched_user_id'] ?? 0); ?>"
                        data-client-name="<?php echo htmlspecialchars($ao['customer_name']); ?>"
                        <?php echo ($selectedAssetId === (int)$ao['id']) ? 'selected' : ''; ?>>
                  <?php echo htmlspecialchars($ao['asset_id']); ?> — <?php echo htmlspecialchars($ao['asset_type']); ?> — <?php echo htmlspecialchars($ao['customer_name']); ?>
                </option>
              <?php
                endwhile;
              endif;
            ...
            <div id="assetHelp" class="form-text">Select a Client Target Email first; Registered Asset will then show only that client's registered assets.</div>
          </div>
